...
def read_label(dir, task):
    if task == 'node_classification':
        ...
        return labels
    #     nodes = []
    #     with open('./data/edge_index.csv') as ef:
    #         next(ef)
    #         for line in ef.readlines():
    #             nodes.extend(line.strip().split(",")[1:3])
...
#     edges = []
#     fin_edges = open('./data/edge_index.csv')
#     next(fin_edges)
#     for line in fin_edges.readlines():
#         node1, node2 = line.strip().split(",")[1:3]
...
def read_file(args, logger):
    ...
    labels = read_label(directory, task=task)
    edges = read_edges(directory)
    ...
    if args.use_attributes:
        ...
        attributes = np.loadtxt("./data/node_feature.csv", delimiter = ",")
...
def get_data(G, task, args, labels, logger):
    ...
    np.savetxt("./data/All_edges.tsv", set_indices, delimiter = "\t", fmt = '%d')
    # if args.debug:
    #     logger.info(list(G.edges))
    data_list = extract_subgaphs(G, labels, set_indices, prop_depth=args.prop_depth, layers=args.layers,
                                 feature_flags=feature_flags, task=task,
                                 max_sprw=(args.max_sp, args.rw_depth), parallel=args.parallel, logger=logger, debug=args.debug)
    train_set, val_set, test_set, train_indices, val_indices, test_indices = split_datalist(data_list, (train_mask, val_test_mask))
    # if args.debug:
    ...
    return (train_loader, val_loader, test_loader), (train_indices, val_indices, test_indices), len(np.unique(labels))
...
def generate_set_indices_labels(G, task, test_ratio, data_usage=1.0):
    ...
    neg_test_size = int(test_ratio * neg_edges.shape[0])
    ...
    test_neg_indices = list(range(n_pos_edges, n_pos_edges + neg_test_size))  # pick first pos_test_size neg edges for test
    test_mask = get_mask(test_pos_indices + test_neg_indices, length=n_pos_edges + neg_edges.shape[0])
    train_mask = np.ones_like(test_mask) - test_mask
    labels = np.concatenate([np.ones((n_pos_edges, )), np.zeros((neg_edges.shape[0], ))]).astype(np.int32)
    ...
    permutation = np.random.permutation(n_pos_edges + neg_edges.shape[0])
...
def sample_pos_neg_sets(G, task, data_usage=1.0):
    ...
    neg_edges = np.array(list(nx.complement(G).edges), dtype=np.int32)
...
def split_datalist(data_list, masks):
    ...
    return train_set, val_set, test_set, train_indices, val_indices, test_indices
